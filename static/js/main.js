...
document.addEventListener('DOMContentLoaded', function() {
    ...
    pinButtons.forEach(button => {
        button.addEventListener('click', (event) => {
            ...
            selectElements.forEach(select => { select.addEventListener('change', function() { 
                ...
                fetch('/update-flashcard/', { method: 'POST', headers: { 
                    'Content-Type': 'application/json', 'X-CSRFToken': csrfToken } // Ensure CSRF token is included }, body: JSON.stringify({ flashcardId, isLearned, isPinned }) }) .then(response => response.json()) .then(data => { // Handle the response, update UI if needed }); }); });
        });
    });
});
